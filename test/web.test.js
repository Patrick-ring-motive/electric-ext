...
function colored(text) {
  return colorize(text).filter(({ color }) => color);
}

assert.deepEqual(colored(String.raw`\b B blue Blueberry table`), [
  { text: String.raw`\b`, color: '#4e4eff' },
  { text: 'B', color: '#4e4eff' },
  { text: 'blue', color: '#4e4eff' },
  { text: 'Blueberry', color: '#4e4eff' },
]);
...
assert.deepEqual(colored('apple123 _blue xylophone'), [
  { text: '123', color: '#00bfff' },
  { text: '_', color: hueRotatedGreen('_') },
]);
assert.match(
